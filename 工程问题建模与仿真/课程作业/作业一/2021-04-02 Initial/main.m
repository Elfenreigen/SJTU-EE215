...
while(currentIteration <= Iteration)               %当迭代次数小于最大迭代次数
    ...
    Adaptation = 1./ Adaptation.^2;                                %适应度=成本平方的倒数
   
    [elite,order] = sort(Adaptation,'descend');
    
                      %精英保留
    nextPopulation = zeros(Population, 90);                        %nextPopulation为下一代的种群情况
    nextPopulation(1, :) = currentPopulation(order(1), :);         %前两行因为精英保留
    nextPopulation(2, :) = currentPopulation(order(2), :);
    nextPopulation(3, :) = currentPopulation(order(3), :);
    nextPopulation(4, :) = currentPopulation(order(4), :);
    nextPopulation(5, :) = currentPopulation(order(5), :);
    nextPopulation(6, :) = currentPopulation(order(6), :);
    nextPopulation(7, :) = currentPopulation(order(7), :);
    nextPopulation(8, :) = currentPopulation(order(8), :);
    ...
    for i = 1:Population - 8
        ...
        select(tmp(1)) = select(tmp(1)) + 1;                       %轮盘赌法（iii):个体每被选中一次，其select值加一。除了两个精英，都参与此轮
                                                                   ...
                                                                   %交叉
    startrow = 9;                                                  %新种群中前两个个体均为上一代中适应度最高的，所以新子代从第三行开始。
    while (sum(select) ~= 0)                                 
